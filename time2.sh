...
grep "time a" $2 |awk -F[ '{print $1","$2}' | sed 's/] s//g' | sed 's/Repl time avg:/Re/g'|sed 's/Eval time avg:/Ev/g'|sed 's/Muta time avg:/Mu/g'|sed 's/Cros time avg:/Cr/g'|sed 's/Sele time avg:/Se/g' >> "$tmpfile2"
...
data2 = np.genfromtxt("$tmpfile2", delimiter=',',dtype=None, autostrip=True, names=True)

mean1 = matplotlib.mlab.rec_groupby(data1, ('op',), (('time',np.mean,'time'),))
mean2 = matplotlib.mlab.rec_groupby(data2, ('op',), (('time',np.mean,'time'),))
...
ax.bar(ind+0.35, mean2['time'] ,0.35, label='cpu + gpu')
